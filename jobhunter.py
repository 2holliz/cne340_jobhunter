...
def create_tables(cursor):
    ...
    company varchar (300), Created_at DATE, url varchar(30000), Title LONGBLOB CHARACTER SET utf8, Description LONGBLOB CHARACTER SET utf8); ''')
...
def add_new_job(cursor, jobdetails):
    ...
    Job_id = html2text.html2text(jobdetails['Job_id'])
    company = html2text.html2text(jobdetails['company'])
...
def notify_user(jobdetails):
    # Need to replace with a more effective notification method such as email or messaging service
    print(f"New job posting found: {jobdetails['title']} at {jobdetails['company']}")
...
def check_if_job_exists(cursor, jobdetails):
    ...
    cursor.execute(query, (jobdetails['job_id'],))
...
def delete_job(cursor, jobdetails):
    ...
    old_job = cutoff.date()
    query = "DELETE FROM jobs WHERE Created_at < %s"
    cursor.execute(query, (old_job))
    return query_sql(cursor, query)
